...
int updateDecreaese(vector<pair<int, float> >& vec, int size, pair<int, float> t)
{
    ...
    for (;;)
    {
        if (i == 0) break;
        j = i - 1;
        if (vec.at(j).first == t.first) return 0;
                ...
                vector<vector<int> >& neighbor, string out_search_file, string ground_path, float& tim, float& rec)
{
    ...
    for (int i = 0; i < query_N; i++)
    {
        ...
        updateDecreaese(solution, K, make_pair(st_point,dis));
        flag[st_point] = 1;
        ...
        while (candidate.front().first != -1)
        {
            ...
            //solution.push(top);
            //int temp_return = updateDecreaese(solution, K, top);
            //if (temp_return == -1) break;
            //cout << "sol " << solution.front().first << endl;
            if ((top.second < solution.back().second)) {
                break;
            }
            ...
            for (int j = 0; j < neighbor[curr].size(); j++)
            {
                ...
                if (flag[nei] == 0)
                {
                    ...
                    updateDecreaese(solution, K, make_pair(nei,dis));
