...
//    OracleL2<FloatDataset> oracle(data);
    OracleInnerProduct<FloatDataset> oracle(data);
...
#pragma omp parallel for
        for (int i = 0; i < data.size(); ++i) {
            for (int j = 0; j < control; ++j) {
                ...
                knns[j].update_innerPro(KNN::Element(i, oracle(control_index[j], i)));
            }
#pragma omp critical
            ++progress;
        }
    ...
    NNDescent<OracleInnerProduct<FloatDataset> > nndes(N, K, S, oracle, GRAPH_BOTH);
